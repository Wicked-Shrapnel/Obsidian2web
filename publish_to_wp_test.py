...
║  Publishes, updates, drafts, and auto-uploads images        ║
...
HOW IT WORKS:
  ...
  - Local images (drag-and-drop or pasted into Obsidian) are automatically
    uploaded to the WordPress Media Library and replaced with hosted URLs.
"""
...
# Log file — saves output to Desktop for debugging
LOG_PATH = os.path.expanduser(r"C:\Users\bradl\Desktop\publisher_log.txt")

class Tee(io.TextIOWrapper):
    """Write to both stdout and a log file simultaneously."""
    def __init__(self, buffer, log_path):
...
def upload_image(image_path):
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            print(f"  Uploaded image: {filename} -> {hosted_url}")
            return hosted_url
    except urllib.error.HTTPError as e:
...
def process_images(body, vault_root):
    """
    Find local image references in the note body, upload them to WordPress,
    and replace the local references with hosted URLs.

    Handles both Obsidian embed syntax:  ![[image.png]]
    And standard Markdown syntax:        ![alt](path/to/image.png)
    """
    # Handle Obsidian embed syntax: ![[filename.ext]]
    def replace_obsidian_embed(match):
        filename = match.group(1).strip()
        # Search for the file recursively in the vault
        for root, dirs, files in os.walk(vault_root):
            # Skip hidden folders
            dirs[:] = [d for d in dirs if not d.startswith('.')]
            if filename in files:
                image_path = os.path.join(root, filename)
                hosted_url = upload_image(image_path)
                if hosted_url:
                    return f'![{filename}]({hosted_url})'
        print(f"  Warning: could not find image in vault: {filename}")
        return match.group(0)

    body = re.sub(r'!\[\[([^\]]+)\]\]', replace_obsidian_embed, body)

    # Handle standard Markdown images with local paths: ![alt](path)
...
def build_post_data(note_path, status):
    ...
    # Upload any local images and replace with hosted WordPress URLs
    # Search from vault root (two levels up covers NAS vault structure)
    vault_root = os.path.dirname(os.path.dirname(note_path))
    body = process_images(body, vault_root)
